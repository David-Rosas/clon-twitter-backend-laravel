...
Route::post('/tweets', function (Request $request) {  
   ...
   return Tweet::create(['user_id' => 1, 'body' => $request->body]);
});

Route::get('users/{user}', function (User $user) {
    return $user->only('id', 'name', 'username', 'avatar', 'profile', 'location', 'link', 'linkText', 'created_at');
});

Route::get('tweets/{user}/tweets', function (User $user) {
